...
## Operating information update — 2026-08-24

- 운영 정보의 기준 원문은 사용자가 제공한 `운영 정보.txt`이며 주소, 운영시간, 휴무일, 주차, 대표전화, 상담전화, 네이버 예약, 카카오톡 상담을 사실 그대로 사용한다.
- 방문 안내의 기존 사진 중심 구성을 유지하되 왼쪽 카피 아래에 주소·운영·휴무·주차를 짧은 정보 레일로 추가한다.
- 방문 행동은 네이버 예약, 전화 문의, 카카오톡 상담으로 역할을 분리하고 모바일 하단 전화 CTA를 실제 대표번호에 연결한다.
- 모바일에서는 운영 정보가 한 열로 정돈되고 모든 연락 CTA가 최소 44px 터치 영역을 유지해야 한다.

## Coaching photo cleanup — 2026-08-24

- `1986 코칭팀`의 6장 proof는 기존 2열 × 3행 순서를 유지한다.
- 사진 앞의 팀명·분야 태그와 각 사진 아래 캡션을 제거하고, 카드 테두리·구분선·고정 프레임·배경을 모두 없앤다.
- 사진은 원본 비율 그대로 노출해 모바일에서 이미지 자체만 이어지는 간결한 갤러리로 만든다.
- 상위 `가르치는 사람이 계속 배우는 센터` 제목과 3장 학습 갤러리는 변경하지 않는다.

## Coaching tetris layout — 2026-08-24

- 동일 높이의 2열 grid가 사진 비율 차이만큼 큰 빈칸을 만드는 문제를 제거한다.
- 6장 proof는 원본 비율을 보존하는 CSS multi-column 모자이크로 바꾸고, 모바일 2열·데스크톱 3열로 빈 공간 없이 위에서 아래로 이어 붙인다.
- 캡션·테두리 없는 상태는 유지하고, 6장은 가로·세로 비율을 기준으로 두 열의 총높이가 비슷해지도록 교차 배치한다. 사진 사이 간격은 모바일 8px·데스크톱 12px로 통일한다.
